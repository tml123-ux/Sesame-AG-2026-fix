...
object Log {
    ...
    private val FISHPOND_LOGGER: Logger
    private val SPORTS_LOGGER: Logger
    private val ORCHARD_LOGGER: Logger
    private val STALL_LOGGER: Logger
    private val MEMBER_LOGGER: Logger
    ...
    init {
        ...
        FISHPOND_LOGGER = LoggerFactory.getLogger("fishpond")
        SPORTS_LOGGER = LoggerFactory.getLogger("sports")
        ORCHARD_LOGGER = LoggerFactory.getLogger("orchard")
        STALL_LOGGER = LoggerFactory.getLogger("stall")
        MEMBER_LOGGER = LoggerFactory.getLogger("member")
    ...
    fun fishpond(msg: String) {
        record(msg)
        FISHPOND_LOGGER.debug("{}", msg)
    }

    @JvmStatic
    fun sports(msg: String) {
        record(msg)
        SPORTS_LOGGER.debug("{}", msg)
    }

    @JvmStatic
    fun orchard(msg: String) {
        record(msg)
        ORCHARD_LOGGER.debug("{}", msg)
    }

    @JvmStatic
    fun stall(msg: String) {
        record(msg)
        STALL_LOGGER.debug("{}", msg)
    }

    @JvmStatic
    fun member(msg: String) {
        record(msg)
        MEMBER_LOGGER.debug("{}", msg)
    }
