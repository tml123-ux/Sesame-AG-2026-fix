        ...
        try {
            ...
            for (i in 0..<signRecords.length()) { //遍历签到记录
                ...
                if (signKey == currentSignKey && !signRecord.getBoolean("signed")) {
                    ...
                    GlobalThreadPools.sleepCompat((2000L..5000L).random())
                ...
                for (i in 0..<forestTasksNew.length()) {
                    ...
                    for (j in 0..<taskInfoList.length()) {
                        ...
                        if (TaskStatus.FINISHED.name == taskStatus) {
                            ...
                            var awardResponse: JSONObject? = null
                            for (awardRetry in 1..3) {
                                if (awardRetry > 1) {
                                    Log.record("森林奖励🔄[$taskTitle] 第${awardRetry}次重试领取")
                                    GlobalThreadPools.sleepCompat((2000L..4000L).random())
                                }
                                awardResponse = JSONObject(
                                    AntForestRpcCall.receiveTaskAward(sceneCode, taskType)
                                )
                                if (ResChecker.checkRes(TAG + "领取森林任务奖励失败:", awardResponse)) {
                                    awardSuccess = true
                                    break
                                }
                            }
                            if (awardSuccess) {
                                ...
                                Log.error(TAG, "领取失败(已重试3次): $taskTitle")
                                Log.record(awardResponse.toString())
                            }
                        GlobalThreadPools.sleepCompat((2000L..5000L).random())
                            ...
                            var finishResponse: JSONObject? = null
                            val maxRetries = 3
                            for (finishRetry in 1..maxRetries) {
                                if (finishRetry > 1) {
                                    Log.record("森林任务🔄[$taskTitle] 第${finishRetry}次重试完成")
                                    GlobalThreadPools.sleepCompat((2000L..4000L).random())
                                }
                                count.incrementAndGet()
                                finishResponse = JSONObject(
                                    AntForestRpcCall.finishTask(sceneCode, taskType)
                                )
                                if (ResChecker.checkRes(TAG + "完成森林任务失败:", finishResponse)) {
                                    finishSuccess = true
                                    count.set(0)
                                    break
                                }
                                ...
                                // 不可重试的错误码，立即停止
                                if (errorCode in setOf("400000040", "ILLEGAL_ARGUMENT", "TASK_ID_INVALID")) {
                                    break
                                }
                            }
                            if (finishSuccess) {
